...
export async function updateProfile(formData: FormData) {
  try {
    ...
    if (!validatedFields.success) {
      throw new Error(
        `Validation failed: ${validatedFields.error.flatten().fieldErrors}`
      );
    }
...
export async function deleteTechnology(formData: FormData) {
  try {
    ...
    const data = await readData();
    const technologyIndex = data.cv.technologies.findIndex(
      (t) => t.name === name
    );
...
export async function listWorkExperiences(): Promise<WorkExperience[]> {
  try {
    const data = await readData();
    return data.cv.workExperiences.sort(
      (a, b) =>
        new Date(b.startDate).getTime() - new Date(a.startDate).getTime()
    );
  } catch (error) {
    throw new Error(
      `Failed to list work experiences: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
export async function updateWorkExperience(formData: FormData) {
  try {
    ...
    const data = await readData();
    const workExperienceIndex = data.cv.workExperiences.findIndex(
      (w) => w.id === id
    );
...
export async function listEducations(): Promise<Education[]> {
  try {
    const data = await readData();
    return data.cv.educations.sort(
      (a, b) =>
        new Date(b.startDate).getTime() - new Date(a.startDate).getTime()
    );
  } catch (error) {
    throw new Error(
      `Failed to list educations: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
export async function listCertifications(): Promise<Certification[]> {
  try {
    const data = await readData();
    return data.cv.certifications.sort(
      (a, b) =>
        new Date(b.startDate).getTime() - new Date(a.startDate).getTime()
    );
  } catch (error) {
...
export async function updateCertification(formData: FormData) {
  try {
    ...
    const data = await readData();
    const certificationIndex = data.cv.certifications.findIndex(
      (c) => c.id === id
    );
...
export async function listAwardOrHonors(): Promise<AwardOrHonor[]> {
  try {
    const data = await readData();
    return data.cv.awardOrHonors.sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  } catch (error) {
    throw new Error(
      `Failed to list awards or honors: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
export async function createAwardOrHonor(formData: FormData) {
  try {
    ...
    throw new Error(
      `Failed to create award or honor: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
export async function updateAwardOrHonor(formData: FormData) {
  try {
    ...
    const data = await readData();
    const awardOrHonorIndex = data.cv.awardOrHonors.findIndex(
      (a) => a.id === id
    );
    ...
    throw new Error(
      `Failed to update award or honor: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
export async function deleteAwardOrHonor(formData: FormData) {
  try {
    ...
    throw new Error(
      `Failed to delete award or honor: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
export async function listPublications(): Promise<Publication[]> {
  try {
    const data = await readData();
    return data.cv.publications.sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  } catch (error) {
    throw new Error(
...
export async function createLanguage(formData: FormData) {
  try {
    const rawData = {
      id: crypto.randomUUID(),
      name: formData.get("name") as string,
      proficiency: formData.get("proficiency") as
        | "Beginner"
        | "Intermediate"
        | "Advanced"
        | "Fluent"
